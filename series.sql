...
/*INSERT INTO capitulo (id_serie, n_temporada, n_orden, fecha_estreno, titulo, duracion) VALUES(?,?,?,?,?,?);
INSERT INTO valora (id_serie, n_temporada, n_orden, id_usuario, fecha, valor) VALUES(?,?,?,?,?,?);*/	

#Catálogo
SELECT titulo, n_capitulos
FROM temporada t
...
select AVG(duracion)
	from capitulo c
        ...
        INNER JOIN serie s on c.id_serie=s.id_serie
        WHERE v.n_orden IS NULL AND v.n_temporada IS NULL AND v.id_serie IS NULL AND s.idioma = 'español';

#setFoto
select fotografia from usuario where apellido1='Cabeza';